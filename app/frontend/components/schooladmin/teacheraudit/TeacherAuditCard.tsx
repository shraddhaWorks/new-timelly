...
export default function TeacherAuditCard({
  ...
  return "bg-lime-500 shadow-[0_0_10px_rgba(132,204,22,0.5)]"; // Default for 50-69
};

const barStyle = getScoreColor(score);

  return (
   /* I've updated the outer padding and the form container padding below */

<div className="px-1 md:px-0"> {/* Reduced mobile horizontal padding from px-3 to px-1 */}
  <div className="max-w-full mx-auto rounded-2xl border border-white/10 shadow-lg overflow-hidden bg-white/5 backdrop-blur-xl">
    ...
    {/* MAIN CARD HEADER */}
    <div className="px-3 py-6 sm:px-8 sm:py-6 border-b border-white/10"> {/* Reduced mobile px from 4 to 3 */}
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 lg:gap-8">

        {/* LEFT SECTION: Profile & Name */}
        <div className="flex items-center gap-4 min-w-0">
          <div className="relative flex-shrink-0">
            <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-2xl overflow-hidden border-2 border-white/10 shadow-xl">
              {teacher.photoUrl ? (
                <img
                ...
                <div className="w-full h-full flex items-center justify-center bg-white/10 text-white/70 text-xl lg:text-2xl font-bold">
                  {teacher.name?.charAt(0) || "?"}
                </div>
              )}
            </div>
            <div className="absolute -bottom-1 -right-1 lg:-bottom-2 lg:-right-2 w-6 h-6 lg:w-7 lg:h-7 rounded-full bg-[#1a1a1a] border border-white/20 flex items-center justify-center shadow-lg">
              <Award className="w-3.5 h-3.5 lg:w-4 lg:h-4 text-yellow-400" />
            </div>
          </div>

          <div className="min-w-0">
            <h3 className="text-lg lg:text-xl font-bold text-white tracking-tight truncate">
              {teacher.name ?? "-"}
            </h3>
            <p className="text-xs lg:text-sm text-white/50 font-medium truncate">
              {teacher.subject ?? "-"}
            </p>
            <p className="text-[10px] lg:text-xs text-white/20 font-mono mt-0.5 truncate uppercase">
  ...
  <div className="h-12 w-12 lg:h-14 lg:w-14 flex-shrink-0 rounded-xl lg:rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center">
  ...
  <div className="flex-1 lg:w-[180px]">
    ...
    <div className="flex justify-between items-center mt-1">
       ...
       {/* Optional: Add a small status label */}
          ...
          {/* ACTION BUTTONS */}
          <div className="flex flex-row flex-nowrap lg:flex-row gap-2 sm:gap-3 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0 scrollbar-hide">
            <button
              onClick={onOpenAddGood}
              className={`h-14 lg:h-16 px-3 lg:px-6 rounded-xl flex items-center justify-center gap-2 text-[10px] lg:text-sm font-bold transition-all flex-1 lg:flex-none whitespace-nowrap ${
                isAddFormOpen && addFormMode === "good"
                  ? "bg-lime-400 text-black shadow-[0_0_15px_rgba(163,230,53,0.4)]"
                  : "bg-white/10 border border-white/20 text-white hover:bg-white/20"
              }`}
            >
              <ThumbsUp size={14} className="lg:w-4 lg:h-4" /> <span>Add Good</span>
            </button>
            ...
            >
              <ThumbsDown size={14} className="lg:w-4 lg:h-4" /> <span>Add Bad</span>
            </button>

            <button
              onClick={onToggleHistory}
              className="h-14 lg:h-16 px-3 lg:px-6 rounded-xl bg-white/10 border border-white/20 text-white flex items-center justify-center gap-2 font-bold flex-1 lg:flex-none hover:bg-white/20 transition-all whitespace-nowrap"
            >
      ...
      {/* ADD FORM SECTION */}
      {isAddFormOpen && (
        <div className="mt-6 rounded-2xl bg-white/5 border border-white/10 px-3 py-6 lg:p-9 backdrop-blur-lg"> {/* Reduced px-5 to px-3 for mobile */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">

            {/* LEFT: Category Selection */}
            <div className="space-y-4">
              <h4 className={`text-xs lg:text-sm font-semibold flex items-center gap-2 uppercase tracking-wider ${
                ...
                {categories.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-[10px] lg:text-xs border transition-all ${
                        isSelected
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  className={`w-full lg:w-auto h-12 px-10 rounded-2xl flex items-center justify-center gap-2 font-bold transition-all ${
                    isFormReady
