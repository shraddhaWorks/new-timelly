...
export default function ClassesHandlingCard({ classes }: ClassesHandlingCardProps) {
  return (
    <section className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl overflow-hidden">
      ...
      <div>
        {classes.map((item) => (
          <div key={item.className} className="border-b border-white/10 px-6 py-5 last:border-b-0">
            <div className="flex items-center justify-between gap-4">
              <div className="flex flex-wrap items-center gap-3">
                <p className="text-[16px] font-semibold text-white">{item.className}</p>
                <span className="rounded-xl border border-lime-400/40 bg-lime-400/10 px-3 py-1 text-[12px] font-semibold text-lime-400">
                  {item.subject}
                </span>
              </div>
